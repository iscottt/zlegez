...
const useGame = () => {
  ...
  const initGame = () => {
    ...
    // 随机生成的总块数
    const totalRandomBlockNum = gameConfig.randomBlocks.reduce((pre: number, curr: number) => {
      return pre + curr;
    }, 0);
   ...
   * @param maxY
   */
  const genLevelBlockPos = (blocks: BlockType[], minX: number, minY: number, maxX: number, maxY: number) => {
  ...
  const doClickBlock = (block: BlockType, randomIdx = -1, force = false) => {
    ...
    if (currSlotNum.value >= gameConfig.slotNum || block.status !== 0 || (block.lowerThanBlocks.length > 0 && !force && !isHolyLight.value)) {
      return;
    }
